...
function buildFileName(data: LdData) {
  return `${data.projectCode}_${data.discipline}_ld_${data.revision}.odt`.toLowerCase();
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!metaEntry || !contentEntry || !stylesEntry) {
      return NextResponse.json({ error: "Template ODT inválido." }, { status: 400 });
    }
    ...
    const fileName = buildFileName(payload.ldData);
